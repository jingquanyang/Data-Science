...
FROM Members
    ) lastestjoindate ON Members.joindate= lastestjoindate.maxdate

/* Q7: Produce a list of all members who have used a tennis court.
Include in your output the name of the court, and the name of the member
formatted as a single column. Ensure no duplicate data, and order by
the member name. */
SELECT  CONCAT(f.name, ': ', m.firstname, ', ', m.surname) AS fullname
FROM Members m
...
SELECT  f.name, CONCAT(m.firstname,', ',m.surname) as user_name,
CASE 
...
SELECT facility_name, user_name, cost
FROM (
    SELECT f.name AS facility_name, CONCAT(m.firstname, ' ', m.surname) AS user_name,
